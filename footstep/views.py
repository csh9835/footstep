...
def post_create(request, username):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            post = Post()
            if post.subject != form.cleaned_data['subject'] and owner.post_set.get(subject=form.cleaned_data['subject']): #게시글 제목 중복방지
                messages.error(request, '동일한 제목의 게시글이 있습니다')
                return redirect('footstep:post_modify', username=username, subject=form.cleaned_data['subject'])
...
def post_modify(request, username, subject):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            if post.subject != form.cleaned_data['subject'] and owner.post_set.get(subject=form.cleaned_data['subject']): #게시글 제목 중복방지
                messages.error(request, '동일한 제목의 게시글이 있습니다')
                return redirect('footstep:post_modify', username=username, subject=subject)
            post.subject = form.cleaned_data['subject']
            post.content = form.cleaned_data['content']
            post.author = owner
            try: #기존 카테고리 이름이 있으면
